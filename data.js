...
async function updateManager(id, manager_id) {
  try {
    await conn.query_promisify("Update employees SET ? WHERE", [
      { manager_id },
      { id }
    ]);
...
async function updateRole(id, role_id) {
  try {
    return await conn.query_promisify("Update employees SET ? WHERE ?", [
      { role_id },
      { id }
    ]);
  } catch (err) {
...
async function getAllDepartments() {
  try {
    ...
    console.log(
      `Error in database: getAllDepartments(). \n Error code: ${err.code} \n `
    );
    return Promise.reject(err.code);
  }
}
...
async function addDepartment(department) {
  try {
    await conn.query_promisify("INSERT INTO departments SET ?", { department });
...
async function addRole({ title, salary, department_id }) {
  try {
    const result = await conn.query_promisify("INSERT INTO roles SET ?", {
      title,
      salary,
      department_id
    });
...
async function addEmployee({ last_name, first_name, role_id, manager_id }) {
  try {
    await conn.query_promisify("INSERT INTO employees SET ?", {
      last_name,
      first_name,
      role_id,
      manager_id
...
async function removeDepartment(department_id) {
  try {
    ...
    if (data.affectedRows > 0) result = { removed: true };
